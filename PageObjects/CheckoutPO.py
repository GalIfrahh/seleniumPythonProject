...
class Checkout:
    ...
    def addPaymentFromCheckout(self):
        GenericPO.driver.findElementBy(self, config['CHECKOUT_SCREEN']['LOCATORS']['LOADER'],
                                          By.XPATH)


    def clickOnSubmitOrder(self):
        if GenericPO.driver.waitForInvisibilityOfElem(self, config['CHECKOUT_SCREEN']['LOCATORS']['LOADER']) is True:
    ...
    def enter4DigitsCode(self):
        GenericPO.driver.findElementBy(self, config['CHECKOUT_SCREEN']['LOCATORS']['ENTER_PIN_INPUT'],
                                    By.XPATH).send_keys(config['FORM_PAGE']['DATA']['PIN'])
